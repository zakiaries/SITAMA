...
@section('content')
  ...
  <div class="page-header" style="display:flex;align-items:center;justify-content:flex-end;gap:12px;">
    @if($unreadCount > 0)
      <form method="POST" action="{{ route('dosen.notifikasi.read-all') }}">
        @csrf
  ...
  <div class="alert-box" style="{{ $notif->is_read ? 'opacity:0.65;' : '' }}">
    ...
    <div style="flex:1;">
      <div class="alert-title">
        <a class="alert-link" href="{{ route('dosen.notifikasi.open', $notif->id) }}">{{ $notif->message }}</a>
    ...
    <div class="alert-actions" style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;">
      ...
      @unless($notif->is_read)
        <form method="POST" action="{{ route('dosen.notifikasi.read', $notif->id) }}">
          @csrf
