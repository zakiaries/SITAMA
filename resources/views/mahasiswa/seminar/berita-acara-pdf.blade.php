...
<body>
  ...
  <div class="title">
    ...
    <div class="subtitle">Daftar Hadir Audiens</div>
  </div>
  <hr>

  <table class="info">
    <tr><td class="k">Judul Seminar</td><td class="s">:</td><td>{{ $seminar->title }}</td></tr>
    <tr><td class="k">Penyaji</td><td class="s">:</td><td>{{ $seminar->presenters->map(fn($p) => $p->student->user->name ?? '-')->join(', ') }}</td></tr>
    ...
    <tr><td class="k">Jumlah Hadir</td><td class="s">:</td><td>{{ $seminar->attendances->count() }} audiens</td></tr>
  </table>
  ...
  <table class="foot">
    <tr>
      ...
      <td>
        <div class="muted">{{ ($seminar->witnessed_at ?? now())->translatedFormat('d F Y') }}</div>
        Mengetahui, Dosen Pembimbing (Saksi),
