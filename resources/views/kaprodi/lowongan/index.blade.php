...
<div style="display:flex;justify-content:flex-end;margin-bottom:14px;">
  <a href="{{ route('kaprodi.lowongan.create') }}" class="btn btn-primary btn-sm">+ Tambah Pengumuman</a>
</div>

<form method="GET" action="{{ route('kaprodi.lowongan.index') }}">
  <input type="hidden" name="status" value="{{ $status }}">
  <div class="search-bar">
    <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
    <input name="search" placeholder="Cari judul / perusahaan..." value="{{ request('search') }}">
  </div>
</form>
...
<div class="filter-tabs">
  ...
  <a href="{{ route('kaprodi.lowongan.index', ['status' => $key, 'search' => request('search')]) }}"
  ...
  <div class="low-head">
    <div>
      ...
      <div class="low-meta">
        ...
        @if($job->division) · {{ $job->division }} @endif
